...
| 2026-08-28 | **Type ramp extended to nine levels, set in rem** — Sub 24, Card title 20, Small 15 and Micro 11-caps join the five v1.1 levels, and the whole scale moves from px to rem/clamp so the reader's font-size setting scales every brand surface. Absorbed from vidra-site, where the ramp shipped and is Playwright-gated; two ramps would drift ([guidelines §04](../guidelines/brand-guidelines.md#04--typography)). | **Decided** |
| 2026-08-28 | **The ambient motion exception** — the 300ms cap gains one escape: an infinite ambient animation may ship only with a written argument, recorded where it lives, for why a one-shot cannot express the same thing. The two existing instances (vidra-site's `vd-pulse` version dot, `vd-dash` federation wires) are grandfathered as the reference arguments; all ambient motion stops dead under `prefers-reduced-motion` ([guidelines §09](../guidelines/brand-guidelines.md#09--accessibility)). | **Decided** |
| 2026-08-28 | **Drifted tokens reconciled with the vidra-site copy** — the site's values and names win where the two disagreed, because they are the shipped, measured artifact. Hairlines go alpha: `--brand-hairline` is `rgb(12 33 54 / 0.12)` (composites ≈ #D9DCE0 on Paper and works on white cards, which the old opaque `#E2E6E9` did not) and `--brand-hairline-ink` drops from `.90` to `0.6`. Cyan tokens take the palette's proper names — `--brand-vidra-cyan` / `--brand-deep-cyan` / `--brand-action-cyan` (were `--brand-cyan` / `--brand-cyan-deep` / `--brand-cyan-action`). The site's surface tokens (`--brand-ink-surface` 0.32, `--brand-ink-chip` 0.6, `--brand-vidra-tint` 0.14) and the three `--brand-chrome-*` glass values (0.92 / 0.94 / `saturate(160%) blur(14px)`) enter the canon sheet. Remaining name drift in the site copy (`--brand-paper-hairline` / `--brand-ink-hairline`) is now the copy's defect to fix. | **Decided** |
| 2026-08-28 | **Comparison surfaces are a brand pattern** ([guidelines §10](../guidelines/brand-guidelines.md#10--comparison-surfaces)) — columns of checkable prose, no ticks/crosses/scoring, admissions of parity and concessions kept in, emphasis by heading colour + fill + position (never colour alone), and a visible verification date on every competitor column. Codified from vidra-site's shipped `Comparison` component before a deck or README re-invents it with checkmarks. | **Decided** |
| 2026-08-28 | **Tabular figures are the number voice** ([guidelines §04](../guidelines/brand-guidelines.md#04--typography)) — every stat, price and count sets `tabular-nums` in its own type level; commands and machine identifiers stay system mono, tabular by nature. The restrained answer to the custom-webfont-mono trend, at zero bytes. | **Decided** |
| 2026-08-28 | **One social-card template** ([`assets/social/og-card.mjs`](../assets/social/og-card.mjs)) — the canonical 1200×630 OG card: README banner on a fixed Ink ground, ~1120×550 safe area, rendered by Playwright Chromium to PNG because the crawlers that matter drop SVG cards. The script mechanically refuses a banner carrying a `prefers-color-scheme` switch (the lockup failure class). vidra-site's `public/brand/og-card.png` and the docs' social card regenerate from it, never from per-repo exports. | **Decided** |
| 2026-08-29 | **Status marks are a brand pattern** ([guidelines §11](../guidelines/brand-guidelines.md#11--status-marks)) — a Micro-level label in exactly two values, shipped and planned: the word as the signal, an 8px dot as redundancy (filled Vidra Cyan / hollow hairline ring), never colour alone, and no third value — "in progress" is not a claim this brand makes. Extends the NotYet idiom from "three things we don't do" to a per-claim mark; codified from vidra-site's shipped `StatusMark` on the /scale ledger before a README or deck re-invents the distinction as green ticks. | **Decided** |
